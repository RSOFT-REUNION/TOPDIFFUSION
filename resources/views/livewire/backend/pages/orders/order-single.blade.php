<div>
    <a href="{{ route('bo.orders') }}" class="btn-slate"><i class="fa-solid fa-arrow-left mr-3"></i>Retour à la liste</a>
        ...
        <div class="flex-none w-[400px]">
            <div class="border w-[400px] overflow-hidden rounded-xl">
                <div class="p-5 border-b">
                    ...
                    <div class="mt-5 cursor-pointer bg-slate-100 rounded-lg overflow-hidden p-5 inline-flex justify-center items-center gap-5 w-full">
                        <i class="fa-solid fa-user fa-2x"></i>
                        <div>
                            <p class="font-bold text-lg">{{ $order->getUserName() }} <span class="text-blue-500">#{{$order->getUser()->id }}</span></p>
                            <p class="text-slate-400 text-sm">{{ $order->getUser()->email }}</p>
                        </div>
                    </div>
                    ...
                    <div class="inline-flex justify-between w-full mt-3">
                        <p class="text-slate-400">Commandes validée :</p>
                        <p class="font-title font-medium">0</p>
                    </div>
                    <div class="inline-flex justify-between w-full mt-3">
                        <p class="text-slate-400">Total dépensé depuis la création :</p>
                        <p class="font-title font-medium">0,00 €</p>
                    </div>
                </div>
                <div class="p-5">
            ...
            <div>
                <button wire:click="changeState('ship')" class="mt-3 btn-slate w-full">Passer la commande en livrée</button>
                <button wire:click="changeState('terminate')" class="mt-2 btn-slate w-full">Passer la commande en terminé</button>
            </div>
        </div>
    </div>
</div>
